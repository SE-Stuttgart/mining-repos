...
public class FilePathToClusterMap {
	...
	/**
	 * Insert the given clusterPath into the given clusterHashMap. Splits the
	 * clusterPath into Cluster and Path. Then checks if Path is already
	 * contained. If yes append cluster. If no create key and add cluster.
	 * 
	 * @param clusterAndPath
	 *            - Filepath containing clusternumber and filepath
	 * @param clusterHashMap
	 *            - Hashmap that has key filepath and value: List of all
	 *            clusters this file is part of.
	 */
	 ...
	 * 
	 * @param count
	 *            - compare this value against the current maxOccurence value
	 */
	...
	/**
	 * removes leading and trailing whitespaces from string splits string by
	 * whitespace delimiter
	 * 
	 * @param clusterAndPath
	 *            - takes String in format "2.2 path/folder/filename"
	...
	private String getName(String clusterAndPath) {
		...
		int index = 0;
		if (!clusterAndPath.contains("/")) {
			index = clusterAndPath.indexOf(" ");
			fileName = clusterAndPath.substring(index + 1);
		} else {
			index = clusterAndPath.lastIndexOf("/");
			fileName = clusterAndPath.substring(index + 1);
		}
